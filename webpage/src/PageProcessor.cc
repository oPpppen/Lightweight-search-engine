...
bool hash_exists(std::vector<uint64_t> &simhashes, uint64_t h) {
...
PageProcessor::PageProcessor() {
  //获取所有文件的名称
  std::vector<std::string> files = DirectoryScanner::scan("../stopwords");
...
void PageProcessor::extract_documents(const std::string &dir) {
  ...
  for (const auto &file : files) {
    ...
    XMLElement *channel = doc.FirstChildElement()->FirstChildElement("channel");
    while (channel) {
      ...
      while (item) {
        ...
        if (title) {
          //去掉html标签
          x.title = regex_replace(title->GetText(), reg, "");
        }
...
//构建倒排索引
void PageProcessor::build_inverted_index(const std::string &path) {
  ...
  for (const auto &[id, wordFreq] : index) {
    ...
    for (const auto &[word, TF] : wordFreq) {
      ...
      double IDF = log2(N / (DF + 1)); // 加1平滑，避免除以0
